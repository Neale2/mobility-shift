...
class SignUpForm(forms.Form):
    ...
    age_group = forms.ChoiceField(
        widget=forms.Select(attrs={"class": "age multi input", "id": "age"}),
    ...
    email = forms.EmailField(
        widget=forms.EmailInput(
            attrs={
                "class": "email short-text input info",
                "id": "email",
    ...
    distance = forms.ChoiceField(
        widget=forms.Select(
            attrs={"class": "distance multi input info", "id": "distance"}
        ),
    ...
    vehicle = forms.ChoiceField(
        widget=forms.Select(attrs={"class": "vehicle multi input", "id": "vehicle"}),
        label="Vehicle Type",
        choices=[
            (243, "Petrol"),
            (265, "Diesel"),
            ...
            (98, "Plug-in Hybrid"),
            (19, "Electric"),
            (243, "Other / Don't know"),
        ],
    )
    employer = forms.ModelChoiceField(
        widget=forms.Select(attrs={"class": "employer multi input", "id": "employer"}),
        label="Employer",
        ...
        blank=False,
        initial="None / Other / Prefer Not To Say",
    )
    region = forms.ModelChoiceField(
        widget=forms.Select(attrs={"class": "region multi input", "id": "region"}),
        label="Region",
...
class YesLogForm(forms.Form):
    quantity = forms.IntegerField(
        widget=forms.NumberInput(attrs={"class": "quantity multi input"}),
        label="How many high carbon trips (one way) did you swap this week?",
        min_value=1,
        max_value=14,
    )
    mode = forms.ChoiceField(
        widget=forms.Select(attrs={"class": "mode multi input"}),
        label="What mode of transport did you use?",
        choices=[
            ("walk", "Walk"),
            ("bike", "Bike / Scooter"),
            ("bus", "Bus"),
...
class NoLogForm(forms.Form):
    text_response = forms.CharField(
        widget=forms.TextInput(attrs={"class": "no-log long-text input"}),
        required=False,
        label="Other:",
    )
    captcha = TurnstileField(label="")
...
class UnsubForm(forms.Form):
    response = forms.ChoiceField(
        widget=forms.Select(attrs={"class": "unsub multi input"}),
        label="Do you want to unsubscribe from the program?",
...
class EditProfileForm(forms.ModelForm):
    ...
    employer = forms.ModelChoiceField(
        widget=forms.Select(attrs={"class": "region multi input"}),
        queryset=Employer.objects.annotate(
            priority=Case(
                When(name="None / Other / Prefer Not To Say", then=Value(0)),
                default=Value(1),
    ...
    region = forms.ModelChoiceField(
        widget=forms.Select(attrs={"class": "widget multi input"}),
        queryset=Region.objects.all(),
