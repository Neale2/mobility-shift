...
class YesLogForm(forms.Form):
    ...
    distance = forms.ChoiceField(widget=forms.Select(attrs={'class': "distance multi input info", 'id': 'distance'}), label="What was the distance travelled, one way?", choices=[(500, "0.5km"), (1000, "1km"), (2500, "2.5km"), (5000, "5km"), (10000, "10km"), (25000, "25km"), (50000, "50km")])
    captcha = TurnstileField(label="")
    
    def clean(self):
        cleaned_data = super().clean()
        
        if (cleaned_data.get('quantity') or 0) + (cleaned_data.get('oneway') or 0) < 1:
            raise forms.ValidationError("Please enter at least one round trip or one-way trip!")
