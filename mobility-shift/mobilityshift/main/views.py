            ...
            try:
                ...
                if str(e) == "UNIQUE constraint failed: main_user.email":
                    form.add_error(
                        None,
                        _(
                            "A user with this Email already exists! You might want to check your inbox, including spam."
                        ),
                    )
                    ...
                    form.add_error(
                        None,
                        _(
                            "Unable to save your response at this time - you might want to wait a couple seconds and try again."
                        ),
                    )
                else:
                    form.add_error(
                        None,
                        _(
                            "There's been an unidentified error! Sorry about that. The system error message is: "
                            + str(e)
                        ),
                    )
        ...
        if form.is_valid():
            ...
            try:
                ...
                emissions_saved = int(
                    ...
                    * user.distance
                    * (user.vehicle - mode_emissions[form.cleaned_data["mode"]])
                    / 1000
                )
        ...
        if form.is_valid():
            ...
            try:
                if form.cleaned_data["response"] == "yes":
                    ...
                    for trip in trips:
                        tripdata = DeletedTrip(
                            user=userdata,
                            text_response=trip.text_response,
                            log_time=trip.log_time,
                            mode=trip.mode,
                ...
                if str(e) == "database is locked":
                    form.add_error(
                        None,
                        _(
                            "Unable to unsubscribe at this time since the database is in use - you might want to wait a couple seconds and try again."
                        ),
                    )
                else:
                    form.add_error(
