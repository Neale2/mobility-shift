...
def email_user(imp_user):
    """
    Slightly messy hack to send an email to a specific user for demos.
    """
    ...
    def send(user):
        ...
        response = send_email(
            user.email, "It's your weekly logging time!", html_body, str(user.uuid)
        )
...
def email_users():
    """
    Send weekly email to all users to log their trips.
    """
    template = get_template("log-email.html")
...
def make_spreadsheet():
    """
    Generate a spreadsheet for internal use.
    """
    ...
    email_response = send_email(
        "arturo.neale@gmail.com",
        "Daily Database Dump",
        "Hi! Here's the trips database from today: "
        + download_url
